        ...
        public void RefreshEntitiesInParentScope(IEnumerable entities)
        {
			...
			foreach (IDbContext contextInCurrentScope in _dbContexts.InitializedDbContexts.Values)
            {
                ...
                foreach (var toRefresh in entities)
                {
                    ...
                    if (contextInCurrentScope.TryGetObjectStateEntry(toRefresh, out stateInCurrentScope))
                    {
                        ...
                        if (correspondingParentContext.TryGetObjectStateEntry(key, out stateInParentScope))
                        {
...
" + Environment.StackTrace;
        ...
        public Core.Repository.IRepository<TEntity> GetRepository<TEntity>() where TEntity : AggregateRoot<TEntity>
        {
